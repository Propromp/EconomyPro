                ...
                when (args[0]) {
                    "help" -> {
                        sender.sendMessage(
                            "$dgray----------[$red EconomyProHELP$dgray -$white /bank $dgray]----------\n" +
                                    "$gold/bank help$dgray»$white ${Main.lang.get(sender,"command.bank.help.help")}\n" +
                                    "$gold/bank select [bankName]$dgray»$white ${Main.lang.get(sender,"command.bank.help.select")}\n" +
                                    "$gold/bank info [bankName]$dgray»$white ${Main.lang.get(sender,"command.bank.help.info")}\n" +
                                    "$gold/bank delete [bankName]$dgray»$white ${Main.lang.get(sender,"command.bank.help.delete")}\n" +
                                    "$gold/bank create$dgray»$white ${Main.lang.get(sender,"command.bank.help.create")}\n" +
                                    "$dgray---------------------------------------------"
                        )
                        return true
                    }
                    "select" -> {
                        ...
                        return if (Main.economy.hasAccount(args[1])) {
                            if (Main.economy.getAccount(args[1])!!.isMemberOrOwner(sender)) {
                                ...
                                sender.sendMessage("${aqua}${Main.lang.get(sender,"command.bank.select.2").replace("%val1%",args[1])}")
                                true
                            } else {
                            ...
                            Main.economy.getDefaultAccount(sender)?.let {
                                sender.sendMessage(
                                    "$dgray[$red ${Main.lang.get(sender,"command.bank.info.title")}$dgray - $white${args[1]} $dgray]\n" +
                                            ...
                                            "   ${gold}${Main.lang.get(sender,"word.owner")}$dgray»$white ${it.player.name}\n" +
                        ...
                        if (Main.economy.hasAccount(args[1])) {
                            if (Main.economy.getAccount(args[1])!!.isMemberOrOwner(sender)) {
                                if (args.size > 2) {
                                    ...
                                    sender.sendMessage("${Main.lang.get(sender,"command.bank.delete.3").replace("%val1%",args[1])}")
                                    sender.sendMessage("${dred}${Main.lang.get(sender,"command.bank.delete.4").replace("%val1%",args[1])}")
                    ...
                    "create" -> {
                        ...
                        return if (!Main.economy.hasAccount(args[1])) {
                            ...
                            sender.sendMessage("${aqua}${Main.lang.get(sender,"command.bank.create.2").replace("%val1%",args[1])}")
                            true
                        } else {
                    ...
                    "invite" -> {
                        ...
                        Main.economy.getAccount(args[1])?.let { account ->
                            Bukkit.getPlayer(args[2])?.let { target ->
                                ...
                                sender.sendMessage("${Main.lang.get(sender,"command.bank.invite.1").replace("%val1%",target.name).replace("%val2%",account.name)}")
                                target.sendMessage("${Main.lang.get(sender,"command.bank.invite.2").replace("%val1",account.name)}")
                                var text = TextComponent("   ")
                                var join = TextComponent("${Main.lang.get(sender,"command.bank.invite.3")}")
                                join.clickEvent =
                    ...
                    "leave" -> {
                        ...
                        Main.economy.getAccount(args[1])?.let { account ->
                            if (account.isMember(sender)) {
                                ...
                                sender.sendMessage("${aqua}${Main.lang.get(sender,"command.bank.leave.1").replace("%val1%",account.name)}")
